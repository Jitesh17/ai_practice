...
def main(episodes, render, monitor):
    ...
    learning_decay = lambda lr, t: max(0.01, min(lr, lr - math.log10((t + 1) / 400)/50))
    epsilon_decay = lambda eps, t: max(0.1, min(eps, eps - math.log10((t + 1) / 500)/50))
    ...
    env.close()
    env = gym.make("CartPole-v0") 
    env = gym.wrappers.Monitor(env, "recording_e",force=True,
                               resume=True,
                            #    write_upon_reset=True, 
